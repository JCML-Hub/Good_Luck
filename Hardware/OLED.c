...
void OLED_WR_Byte(unsigned char dat, unsigned char cmd)
{
  ...
  if (cmd == OLED_DATA)
    HAL_I2C_Mem_Write(&ENABLEOLED ,0x78,0x40,I2C_MEMADD_SIZE_8BIT,&dat,1,0x100);
  else
    HAL_I2C_Mem_Write(&ENABLEOLED ,0x78,0x00,I2C_MEMADD_SIZE_8BIT,&dat,1,0x100);
}
...
void OLED_Init(void)
{
  ...
  OLED_WR_Byte(0xA1, OLED_CMD); //设置左 右方向，0xA1正常 0xA0左右反置
...
void OLED_ShowFloatNum(unsigned char x, unsigned char y, float num, unsigned char len, unsigned char size2)
{
  ...
  Num_int = (int)num;
  Num_float = (int)num;
  Num_float = (int)((num-(float)Num_float) * oled_pow(10, len%10+1));
